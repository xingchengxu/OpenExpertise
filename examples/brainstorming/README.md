...
```mermaid
flowchart TD
  subgraph phase_seed["seed"]
    load_seed["load_seed"]:::tool
  end
  subgraph phase_diverge["diverge"]
    diverge[["diverge
⟳ for each $.angles"]]:::cli-agent
  end
  subgraph phase_converge["converge"]
    cluster("cluster"):::agent
  end
  subgraph phase_critique["critique"]
    critique[["critique
⟳ for each $.clusters"]]:::cli-agent
  end
