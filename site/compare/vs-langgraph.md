...
| Evolution loop     | `oe evolve` reads event log, proposes YAML graph patches; `oe evolve --runs` finds patterns recurring across runs | No equivalent; graph changes require code edits                             |
| Visualization      | `oe graph` renders the DAG as Mermaid; `oe inspect --html` emits a self-contained run report (no server)          | LangGraph Studio — GUI debugger, requires the Studio/Platform tooling       |
