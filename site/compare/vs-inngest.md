...
| Self-evolution        | `oe evolve` proposes graph changes from runtime traces; `oe evolve --runs` surfaces cross-run patterns   | Not present                                                                        |
| Visualization         | `oe graph` renders the DAG as Mermaid; `oe inspect --html` emits a self-contained run report — no server | Rich observability UIs, but require the Temporal / Inngest server or cloud service |
