...
| MCP server             | Exposes 8 OE tools (incl. `oe_graph`) so Claude Code can orchestrate OE                                                         | Runs inside Claude Code; not callable externally         |
| Visualization          | `oe graph` renders the DAG as Mermaid; `oe inspect --html` emits a shareable run report (DAG colored by node status + timeline) | Live TUI only; no persisted/shareable artifact           |
