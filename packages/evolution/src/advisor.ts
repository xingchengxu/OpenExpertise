...
export class EvolutionAdvisor {
  ...
  async analyzeAcrossRuns(input: CrossRunInput): Promise<EvolutionProposal[]> {
    const systemPath = resolve(HERE, 'prompts/cross-run.md')
    const system = readFileSync(systemPath, 'utf8')
    const userPayload = {
      experience_yaml: input.experienceYamlSource,
      ...
      runs: input.runs.map((r) => ({
        run_id: r.runId,
        event_count: r.runEvents.length,
        state_diff: r.stateDiff,
        // a small slice per run so we don't blow the context budget
        sample_events: r.runEvents.slice(0, 15),
      })),
    }
    const tool: LLMTool = {
      name: 'structured_output',
  ...
  renderMarkdownCrossRun(proposals: EvolutionProposal[], runIds: string[]): string {
    const lines: string[] = []
    lines.push(`# Cross-Run Evolution Proposals (${runIds.length} runs)`)
    lines.push('')
    lines.push(`Runs analyzed: ${runIds.map((r) => `\`${r}\``).join(', ')}`)
    lines.push('')
    lines.push(`Generated by OpenExpertise EvolutionAdvisor. Proposals are prioritized by`)
    lines.push(`STABLE patterns recurring across runs over one-off blips. Review each`)
    lines.push(`suggestion and \`git apply\` the diff blocks you accept; runtime never`)
    lines.push(`auto-applies.`)
    lines.push('')
    if (proposals.length === 0) {
      lines.push('_No proposals generated across these runs._')
      return lines.join('\n') + '\n'
    }
    for (let i = 0; i < proposals.length; i++) {
      const p = proposals[i]!
